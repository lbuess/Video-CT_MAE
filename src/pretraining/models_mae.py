...
from src.pretraining.util import video_vit
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward(self, imgs, mask_ratio=0.75, visualize=False):
        ...
        # visualization from https://github.com/haooooooqi/visualization/tree/main
        if visualize:
            N, T, H, W, p, u, t, h, w = self.patch_info
            pred = pred
            ...
            reconstruct = self.unpatchify(
                ...
                + self.target * (1 - mask.reshape(N, t * h * w, 1))
            )
            masked = self.unpatchify(self.target * (1 - mask.reshape(N, t * h * w, 1)))
            comparison = torch.stack([imgs, masked, reconstruct], dim=1)
